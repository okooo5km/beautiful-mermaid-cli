...
// Wide characters (CJK, fullwidth, etc.) occupy 2 terminal columns. We
// store the glyph in one canvas cell and a zero-width sentinel in the next
// so the column-major grid math stays consistent. Output stitches the
// sentinel back to '' so the printed row matches the virtual layout.
...
export function drawText(
  ...
  // Iterate by code point so combining/surrogate pairs stay together,
  // and use display width so CJK glyphs reserve 2 cells.
  let cursor = start.x
  for (const ch of text) {
    const w = charWidth(ch)
    // When centering math underflows the canvas (e.g. a long CJK edge label
    // on a short edge), skip the off-canvas leading cells rather than indexing
    // a negative column.
    if (cursor < 0 || start.y < 0) {
      cursor += w
      continue
    }
    increaseSize(canvas, cursor + w - 1, start.y)
    const current = canvas[cursor]![start.y]!
    if (forceOverwrite || current === ' ') {
      canvas[cursor]![start.y] = ch
      if (w === 2) canvas[cursor + 1]![start.y] = WIDE_PLACEHOLDER
    }
    cursor += w
  }
}
