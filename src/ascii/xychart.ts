...
import { parseXYChart } from './parsers/xychart.js'
import type { XYChart } from './parsers/xychart-types.js'
import type { AsciiConfig, AsciiTheme, ColorMode, CharRole, Canvas, RoleCanvas } from './types.js'
import { colorizeText } from './ansi.js'
import { getSeriesColor, CHART_ACCENT_FALLBACK } from './parsers/xychart-colors.js'
...
function writeText(canvas: Canvas, roles: RoleCanvas, row: number, startCol: number, text: string, role: CharRole): void {
  ...
  for (const ch of text) {
    ...
    set(canvas, roles, row, cursor, ch, role)
    if (cw === 2) set(canvas, roles, row, cursor + 1, '', role)
    cursor += cw
  }
}
