...
import type { SequenceDiagram, Block } from './parsers/sequence-types.js'
import type { Canvas, AsciiConfig, RoleCanvas, CharRole, AsciiTheme, ColorMode } from './types.js'
...
export function renderSequenceAscii(text: string, config: AsciiConfig, colorMode?: ColorMode, theme?: AsciiTheme): string {
  ...
  /**
   * Draw `text` starting at (x, y) with CJK-aware cell stepping. Optional
   * right bound: cells whose first column would land at or after `xLimit`
   * are dropped (matches the existing per-char loops with `< xLimit` checks).
   */
  function setCText(x: number, y: number, text: string, role: CharRole, xLimit?: number): void {
    let cursor = x
    for (const ch of text) {
      const cw = charWidth(ch)
      if (xLimit !== undefined && cursor >= xLimit) break
      setC(cursor, y, ch, role)
      if (cw === 2) setC(cursor + 1, y, '', role)
      cursor += cw
    }
  }
  ...
  for (let m = 0; m < diagram.messages.length; m++) {
    ...
    if (isSelf) {
      ...
      setCText(labelX, y0 + 1, msg.label, 'text', totalW)
      ...
      for (let lineIdx = 0; lineIdx < msgLines.length; lineIdx++) {
        ...
        setCText(labelStart, y, line, 'text', totalW)
      }
  ...
  for (let b = 0; b < diagram.blocks.length; b++) {
    ...
    for (let lineIdx = 0; lineIdx < hdrLines.length && topY + lineIdx < botY; lineIdx++) {
      ...
      setCText(bLeft + 1, topY + lineIdx, line, 'text', bRight)
    }
    ...
    for (let d = 0; d < block.dividers.length; d++) {
      ...
      if (dLabel) {
        const dStr = `[${dLabel}]`
        setCText(bLeft + 1, dY, dStr, 'text', bRight)
      }
    }
  }
  ...
  for (const np of notePositions) {
    ...
    for (let l = 0; l < np.lines.length; l++) {
      ...
      setCText(np.x + 2, ly, np.lines[l]!, 'text', np.x + np.width - 1)
    }
    // Bottom border
