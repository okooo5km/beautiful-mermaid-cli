...
function drawBoxWithGridDimensions(node: AsciiNode, graph: AsciiGraph): Canvas {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const ch of line) {
      ...
      const y = startY + i
      if (cursor >= 0 && cursor < box.length && y >= 0 && y < box[0]!.length) {
        box[cursor]![y] = ch
        if (cw === 2 && cursor + 1 < box.length) box[cursor + 1]![y] = ''
      }
      cursor += cw
    }
  }
...
export function drawMultiBox(
  ...
  for (let s = 0; s < sections.length; s++) {
    ...
    for (const line of lines) {
      ...
      for (const ch of line) {
        const cw = charWidth(ch)
        canvas[cursor]![row] = ch
        if (cw === 2) canvas[cursor + 1]![row] = ''
        cursor += cw
      }
      row++
    }
...
export function drawSubgraphLabel(sg: AsciiSubgraph, graph: AsciiGraph): [Canvas, DrawingCoord] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const ch of line) {
      const cw = charWidth(ch)
      if (cursor < width && labelY < height) {
        canvas[cursor]![labelY] = ch
        if (cw === 2 && cursor + 1 < width) canvas[cursor + 1]![labelY] = ''
      }
      cursor += cw
    }
  }
