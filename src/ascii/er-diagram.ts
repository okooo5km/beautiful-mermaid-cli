...
import type { ErDiagram, ErEntity, ErAttribute, Cardinality } from './parsers/er-types.js'
    ...
    if (sameRow) {
      ...
      if (rel.label) {
        ...
        for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
          ...
          const lineDw = displayWidth(line)
          const labelStart = Math.max(startX, gapMid - Math.floor(lineDw / 2))
          const labelY = lineY + 1 + lineIdx
          // Ensure canvas is tall enough
          increaseSize(canvas, Math.max(labelStart + lineDw, 1), Math.max(labelY + 1, 1))
          increaseRoleCanvasSize(rc, Math.max(labelStart + lineDw, 1), Math.max(labelY + 1, 1))
          let cursor = labelStart
          for (const ch of line) {
            const cw = charWidth(ch)
            if (cursor >= startX && cursor <= endX) {
              setC(cursor, labelY, ch, 'text')
              if (cw === 2 && cursor + 1 <= endX) setC(cursor + 1, labelY, '', 'text')
            }
            cursor += cw
          }
        }
        ...
        for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
          ...
          if (y >= 0) {
            ...
            for (const ch of line) {
              const cw = charWidth(ch)
              if (cursor >= 0) {
                increaseSize(canvas, cursor + cw, y + 1)
                increaseRoleCanvasSize(rc, cursor + cw, y + 1)
                setC(cursor, y, ch, 'text')
                if (cw === 2) setC(cursor + 1, y, '', 'text')
              }
              cursor += cw
            }
          }
